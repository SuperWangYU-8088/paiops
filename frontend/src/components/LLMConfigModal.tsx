...
const LLMConfigModal: React.FC<LLMConfigModalProps> = ({ visible, onClose }) => {
  ...
  const handleOpenForm = (config?: LLMGlobalConfig) => {
    ...
    if (config) {
      ...
      form.setFieldsValue({
        ...
        // 安全接口不会回填密钥；留空提交表示继续使用服务端已保存的密钥。
        apiKey: '',
        model: config.model,
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 300,
      ellipsis: true,
      render: (_: unknown, record: LLMGlobalConfig) => getCapabilitySummary(record)
    },
    {
      title: '操作',
      key: 'action',
      width: 96,
      render: (_: unknown, record: LLMGlobalConfig) => (
        <Space size="small">
          <Button
            type="link"
            size="small"
          ...
          >
            ...
            <Form.Item
              ...
              rules={[{
                ...
                message: '新增配置时请输入 API 密钥'
              }]}
              extra={editingConfig?.apiKeyConfigured ? '已安全保存密钥；留空表示不修改。' : undefined}
            >
              <Input.Password placeholder={editingConfig ? '留空表示保留原密钥' : '输入 API 密钥'} />
            </Form.Item>
            ...
            >
              <Input placeholder="例如: deepseek-v4-flash、deepseek-v4-pro、gpt-4.1" />
            </Form.Item>
