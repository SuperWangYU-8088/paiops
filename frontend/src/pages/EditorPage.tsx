...
import { Button, Input, InputNumber, Form, message, Checkbox, Select, Modal, List, Popconfirm, Switch, Tooltip, Upload } from 'antd';
...
const EditorPage = () => {
  ...
  const { nodes, edges, currentWorkflowId, setCurrentWorkflowId, selectedNode, setNodes, setEdges, deleteNode } = useWorkflowStore();
  ...
  const [connectorCredentials, setConnectorCredentials] = useState<ConnectorCredential[]>([]);
  ...
  const handleDeleteSelectedNode = useCallback(() => {
    const currentNode = useWorkflowStore.getState().selectedNode;
    if (!currentNode) return;

    const nodeType = String(currentNode.data?.type || '');
    if (nodeType === 'input' || nodeType === 'output') {
      message.info('输入和输出节点为流程必需节点，不能删除');
      return;
    }
  ...
  useEffect(() => {
    const handleDeleteKey = (event: KeyboardEvent) => {
      ...
      const isEditing = Boolean(
        ...
        target?.closest('input, textarea, select, [contenteditable="true"], [role="textbox"], .ant-select, .ant-input-number')
      );
      if (isEditing || !useWorkflowStore.getState().selectedNode) return;
  ...
  useEffect(() => {
    getCredentials()
      .then((result) => {
        if (result.code === 200) {
          setConnectorCredentials(result.data);
        }
      })
      .catch(() => {
  ...
  const getNodeOutputParams = (nodeType: string): string[] => {
    switch (nodeType) {
      ...
      case 'http_health_check':
        return ['healthy', 'statusCode', 'responseTimeMs', 'body', 'error'];
      case 'prometheus_query':
        return ['success', 'resultType', 'resultCount', 'result', 'responseTimeMs', 'error'];
      case 'loki_query':
        return ['success', 'resultType', 'streamCount', 'result', 'responseTimeMs', 'error'];
      case 'kubernetes_query':
        return ['success', 'count', 'items', 'resourceVersion', 'error'];
      case 'kubernetes_scale':
      case 'kubernetes_restart':
      case 'kubernetes_rollback':
        return ['success', 'dryRun', 'changeId', 'before', 'after'];
      case 'host_resource_check':
        return ['healthy', 'cpuUsagePercent', 'memoryUsagePercent', 'disks'];
      case 'database_health_check':
        return ['healthy', 'databaseProduct', 'databaseVersion', 'responseTimeMs', 'error'];
      case 'webhook_notify':
        return ['delivered', 'status', 'statusCode', 'payload', 'responseTimeMs', 'error'];
      case 'change_gate':
        return ['changeGatePassed', 'changeGatePassedAt', 'gateNodeId'];
      case 'manual_approval':
        return ['approved', 'approvalId', 'approvedBy', 'approvedAt'];
      default:
        return ['output'];
    }
  };
  ...
  const handleSaveLlmConfig = () => {
    ...
    // Runbook 只保存全局配置 ID，禁止把模型密钥写入工作流 JSON。
    if (!llmConfig.configId) {
      message.warning('请先选择全局模型配置；Runbook 不允许保存 API 密钥');
      return;
    }
  ...
  const handleReferenceImageUpload = async (file: File) => {
    ...
    try {
      ...
      message.error(error instanceof Error ? error.message : '原图上传失败');
    }
  };
    ...
    <div className="workflow-editor-shell">
      ...
      <header className="workflow-topbar">
        <div className="workflow-title-group">
          ...
          <div>
            <div className="workflow-brand">PaiOps</div>
            <Input
              value={workflowName}
          ...
          {selectedNode ? (
            <div className="workflow-config-body">
              <div className="workflow-selected-card">
                ...
                {!['input', 'output'].includes(selectedNodeType) && (
                  <Tooltip title="删除节点">
                    <Button
                      className="workflow-selected-delete"
                      type="text"
                      danger
                      icon={<DeleteOutlined />}
                      aria-label="删除节点"
                ...
                {isLlmNodeType(selectedNodeType) && (
                  <Form layout="vertical" className="workflow-config-form">
                    ...
                    {/* 密钥统一由全局配置中心保管，Runbook 不提供明文输入。 */}
                    {!llmConfig.configId && (
                      <div className="workflow-config-warning">
                        请先在“全局模型配置”中保存 API 地址和密钥，然后在这里选择配置。
                        Runbook 不允许保存任何模型密钥明文。
                      </div>
                    )}
                ...
                {selectedNode.data?.type === 'tts' && (
                  <Form layout="vertical" className="workflow-config-form">
                    ...
                    <div className="workflow-config-section">
                      ...
                      {!ttsConfig.configId && (
                        <div className="workflow-config-warning">
                          请先在“全局模型配置”中保存 TTS API 密钥，再选择对应配置。
                        </div>
                      )}
                    ...
                    {selectedNodeType === 'http_health_check' && (
                      <Form layout="vertical" className="workflow-config-form">
                        <Form.Item label="检查地址" required>
                          <Input
                        ...
                        <Form.Item label="请求方法">
                          <Select
                            ...
                            onChange={(value) => updateSelectedNodeData({ method: value })}
                          />
                        </Form.Item>
                        <Form.Item label="加密凭证">
                          <Select
                            allowClear
                            value={selectedNode.data?.credentialId as number | undefined}
                            placeholder="可选：Authorization 凭证"
                            options={connectorCredentials
                              .filter((credential) => ['webhook', 'http'].includes(credential.connectorType))
                              .map((credential) => ({ value: credential.id, label: credential.name }))}
                            onChange={(value) => updateSelectedNodeData({ credentialId: value })}
                          />
                        </Form.Item>
                        <div className="workflow-config-inline">
                          <Form.Item label="超时（秒）">
                            <InputNumber
                              min={1}
                              max={30}
                              value={Number(selectedNode.data?.timeoutSeconds ?? 10)}
                              onChange={(value) => updateSelectedNodeData({ timeoutSeconds: value ?? 10 })}
                            />
                          </Form.Item>
                          <Form.Item label="最小状态码">
                            <InputNumber
                              min={100}
                              max={599}
                              value={Number(selectedNode.data?.expectedStatusMin ?? 200)}
                              onChange={(value) => updateSelectedNodeData({ expectedStatusMin: value ?? 200 })}
                            />
                          </Form.Item>
                          <Form.Item label="最大状态码">
                            <InputNumber
                              min={100}
                              max={599}
                              value={Number(selectedNode.data?.expectedStatusMax ?? 399)}
                              onChange={(value) => updateSelectedNodeData({ expectedStatusMax: value ?? 399 })}
                            />
                          </Form.Item>
                        </div>
                        <Form.Item label="响应正文">
                          <Switch
                            checked={Boolean(selectedNode.data?.includeBody)}
                            checkedChildren="保留"
                            unCheckedChildren="忽略"
                            onChange={(checked) => updateSelectedNodeData({ includeBody: checked })}
                          />
                        </Form.Item>
                        <Form.Item label="请求头（JSON）">
                          <Input.TextArea
                            ...
                            onChange={(event) => updateSelectedNodeData({ headers: event.target.value })}
                          />
                        </Form.Item>
                      </Form>
                    )}

                    {selectedNodeType === 'prometheus_query' && (
                      <Form layout="vertical" className="workflow-config-form">
                        <Form.Item label="Prometheus 地址" required>
                          <Input
                            value={String(selectedNode.data?.endpoint || '')}
                            placeholder="http://prometheus:9090"
                            onChange={(event) => updateSelectedNodeData({ endpoint: event.target.value })}
                          />
                        </Form.Item>
                        <Form.Item label="PromQL" required>
                          <Input.TextArea
                            ...
                            placeholder="up == 0"
                            onChange={(event) => updateSelectedNodeData({ query: event.target.value })}
                          />
                        </Form.Item>
                        <Form.Item label="加密凭证">
                          <Select
                            allowClear
                            value={selectedNode.data?.credentialId as number | undefined}
                            placeholder="Prometheus 只读凭证"
                            options={connectorCredentials
                              .filter((credential) => credential.connectorType === 'prometheus')
                              .map((credential) => ({ value: credential.id, label: credential.name }))}
                            onChange={(value) => updateSelectedNodeData({ credentialId: value })}
                          />
                        </Form.Item>
                    ...
                    {selectedNodeType === 'loki_query' && (
                      <Form layout="vertical" className="workflow-config-form">
                        <Form.Item label="Loki 地址">
                          <Input
                            value={String(selectedNode.data?.endpoint || '')}
                        ...
                        <Form.Item label="加密凭证">
                          <Select
                            allowClear
                            value={selectedNode.data?.credentialId as number | undefined}
                            placeholder="Loki 只读凭证"
                            options={connectorCredentials
                              .filter((credential) => credential.connectorType === 'loki')
                              .map((credential) => ({ value: credential.id, label: credential.name }))}
                            onChange={(value) => updateSelectedNodeData({ credentialId: value })}
                          />
                        </Form.Item>
                        ...
                        <div className="workflow-config-inline">
                          <Form.Item label="最大日志流">
                            <InputNumber
                              min={1}
                              max={1000}
                              value={Number(selectedNode.data?.limit ?? 100)}
                              onChange={(value) => updateSelectedNodeData({ limit: value ?? 100 })}
                            />
                          </Form.Item>
                          <Form.Item label="方向">
                            <Select
                              value={String(selectedNode.data?.direction || 'backward')}
                              options={[
                                { value: 'backward', label: '从新到旧' },
                                { value: 'forward', label: '从旧到新' }
                              ]}
                              onChange={(value) => updateSelectedNodeData({ direction: value })}
                            />
                          </Form.Item>
                          <Form.Item label="超时（秒）">
                            <InputNumber
                    ...
                    {selectedNodeType === 'kubernetes_query' && (
                      <Form layout="vertical" className="workflow-config-form">
                        <Form.Item label="Kubernetes 凭证" required>
                          <Select
                            value={selectedNode.data?.credentialId as number | undefined}
                            placeholder="选择只读 ServiceAccount"
                            options={connectorCredentials
                              .filter((credential) => credential.connectorType === 'kubernetes')
                              .map((credential) => ({ value: credential.id, label: credential.name }))}
                            onChange={(value) => updateSelectedNodeData({ credentialId: value })}
                          />
                        </Form.Item>
                        ...
                        <div className="workflow-config-inline">
                          <Form.Item label="命名空间">
                            <Input
                              value={String(selectedNode.data?.namespace || 'default')}
                              onChange={(event) => updateSelectedNodeData({ namespace: event.target.value })}
                            />
                          </Form.Item>
                          <Form.Item label="资源类型">
                            <Select
                              value={String(selectedNode.data?.resource || 'pods')}
                              options={[
                              ...
                              onChange={(value) => updateSelectedNodeData({ resource: value })}
                            />
                          </Form.Item>
                          <Form.Item label="超时（秒）">
                            <InputNumber
                        ...
                        <Form.Item label="资源名称">
                          <Input
                            value={String(selectedNode.data?.resourceName || '')}
                            placeholder="留空查询列表"
                            onChange={(event) => updateSelectedNodeData({ resourceName: event.target.value })}
                          />
                        </Form.Item>
                        <Form.Item label="标签选择器">
                          <Input
                            value={String(selectedNode.data?.labelSelector || '')}
                            placeholder="app=api,environment=prod"
                            onChange={(event) => updateSelectedNodeData({ labelSelector: event.target.value })}
                          />
                        </Form.Item>
                      </Form>
                    )}

                    {['kubernetes_scale', 'kubernetes_restart', 'kubernetes_rollback'].includes(selectedNodeType) && (
                      <Form layout="vertical" className="workflow-config-form">
                        <div className="workflow-field-hint workflow-risk-hint">
                          高风险动作：必须连接“人工审批”节点。默认仅执行 Kubernetes dry-run，
                          关闭演练模式后才会真正变更集群。
                        </div>
                        <Form.Item label="Kubernetes 凭证" required>
                          <Select
                            value={selectedNode.data?.credentialId as number | undefined}
                            placeholder="选择最小权限 ServiceAccount"
                            options={connectorCredentials
                              .filter((credential) => credential.connectorType === 'kubernetes')
                              .map((credential) => ({ value: credential.id, label: credential.name }))}
                            onChange={(value) => updateSelectedNodeData({ credentialId: value })}
                        ...
                        <div className="workflow-config-inline">
                          ...
                          <Form.Item label="Deployment" required>
                            <Input
                              value={String(selectedNode.data?.deployment || '')}
                              onChange={(event) => updateSelectedNodeData({ deployment: event.target.value })}
                            />
                          </Form.Item>
                        </div>
                        {selectedNodeType === 'kubernetes_scale' && (
                          <Form.Item label="目标副本数（0-200）" required>
                            <InputNumber
                              min={0}
                              max={200}
                              value={Number(selectedNode.data?.replicas ?? 1)}
                              onChange={(value) => updateSelectedNodeData({ replicas: value ?? 1 })}
                            />
                          </Form.Item>
                        )}
                        {selectedNodeType === 'kubernetes_restart' && (
                          <Form.Item label="幂等变更标识">
                            <Input
                              value={String(selectedNode.data?.changeId || '')}
                              placeholder="留空时使用执行任务 ID"
                              onChange={(event) => updateSelectedNodeData({ changeId: event.target.value })}
                            />
                          </Form.Item>
                        )}
                        {selectedNodeType === 'kubernetes_rollback' && (
                          <>
                            <Form.Item label="容器名称" required>
                              <Input
                                value={String(selectedNode.data?.container || '')}
                                onChange={(event) => updateSelectedNodeData({ container: event.target.value })}
                              />
                            </Form.Item>
                            <Form.Item label="目标镜像" required>
                              <Input
                                value={String(selectedNode.data?.targetImage || '')}
                                placeholder="registry.example.com/app:v1.2.3"
                                onChange={(event) => updateSelectedNodeData({ targetImage: event.target.value })}
                              />
                            </Form.Item>
                          </>
                        )}
                        <div className="workflow-config-inline">
                          <Form.Item label="演练模式">
                            <Switch
                              checked={selectedNode.data?.dryRun !== false}
                              checkedChildren="Dry Run"
                              unCheckedChildren="真实执行"
                              onChange={(checked) => updateSelectedNodeData({ dryRun: checked })}
                            />
                          </Form.Item>
                          <Form.Item label="超时（秒）">
                            <InputNumber
                              min={1}
                              max={60}
                              value={Number(selectedNode.data?.timeoutSeconds ?? 20)}
                              onChange={(value) => updateSelectedNodeData({ timeoutSeconds: value ?? 20 })}
                            />
                          </Form.Item>
                        </div>
                      </Form>
                    ...
                    {selectedNodeType === 'host_resource_check' && (
                      <Form layout="vertical" className="workflow-config-form">
                        <div className="workflow-config-inline">
                          <Form.Item label="CPU 上限（%）">
                            <InputNumber
                              min={1}
                              max={100}
                              value={Number(selectedNode.data?.maxCpuPercent ?? 90)}
                              onChange={(value) => updateSelectedNodeData({ maxCpuPercent: value ?? 90 })}
                            />
                          </Form.Item>
                          <Form.Item label="内存上限（%）">
                            <InputNumber
                              min={1}
                              max={100}
                              value={Number(selectedNode.data?.maxMemoryPercent ?? 90)}
                              onChange={(value) => updateSelectedNodeData({ maxMemoryPercent: value ?? 90 })}
                            />
                          </Form.Item>
                        </div>
                      </Form>
                    ...
                    {selectedNodeType === 'database_health_check' && (
                      <Form layout="vertical" className="workflow-config-form">
                        <Form.Item label="MySQL 加密凭证" required>
                          <Select
                            value={selectedNode.data?.credentialId as number | undefined}
                            placeholder="选择只读数据库账号"
                            options={connectorCredentials
                              .filter((credential) => credential.connectorType === 'mysql')
                              .map((credential) => ({ value: credential.id, label: credential.name }))}
                            onChange={(value) => updateSelectedNodeData({ credentialId: value })}
                          />
                        </Form.Item>
                    ...
                    {selectedNodeType === 'webhook_notify' && (
                      <Form layout="vertical" className="workflow-config-form">
                        ...
                        <Form.Item label="加密凭证">
                          <Select
                            allowClear
                            value={selectedNode.data?.credentialId as number | undefined}
                            placeholder="Webhook Authorization 凭证"
                            options={connectorCredentials
                              .filter((credential) => credential.connectorType === 'webhook')
                              .map((credential) => ({ value: credential.id, label: credential.name }))}
                            onChange={(value) => updateSelectedNodeData({ credentialId: value })}
                          />
                        </Form.Item>
                        <Form.Item label="通知内容" required>
                          <Input.TextArea
                            ...
                            placeholder="支持填写上游节点传入的消息"
                            onChange={(event) => updateSelectedNodeData({ message: event.target.value })}
                          />
                        </Form.Item>
                        <Form.Item label="告警级别">
                          <Select
                            value={String(selectedNode.data?.severity || 'info')}
                            options={[
                              { value: 'info', label: '信息' },
                              { value: 'warning', label: '警告' },
                              { value: 'critical', label: '严重' }
                            ]}
                            onChange={(value) => updateSelectedNodeData({ severity: value })}
                          />
                        </Form.Item>
                        <Form.Item label="演练模式">
                          <Switch
                        ...
                        <Form.Item label="附带脱敏上下文">
                          <Switch
                            checked={Boolean(selectedNode.data?.includeContext)}
                            checkedChildren="开启"
                            unCheckedChildren="关闭"
                            onChange={(checked) => updateSelectedNodeData({ includeContext: checked })}
                          />
                        </Form.Item>
                        <Form.Item label="超时（秒）">
                          <InputNumber
                    ...
                    {selectedNodeType === 'change_gate' && (
                      <Form layout="vertical" className="workflow-config-form">
                        <Form.Item label="批准口令" required>
                          <Input.Password
                            value={String(selectedNode.data?.requiredPhrase || 'APPROVE')}
                            onChange={(event) => updateSelectedNodeData({ requiredPhrase: event.target.value })}
                          />
                        </Form.Item>
                      </Form>
                    )}

                    {selectedNodeType === 'manual_approval' && (
                      <Form layout="vertical" className="workflow-config-form">
                        <Form.Item label="审批原因">
                          <Input.TextArea
                            rows={4}
                            value={String(selectedNode.data?.approvalReason || '')}
                            placeholder="说明变更目标、影响范围和预期结果"
                            onChange={(event) => updateSelectedNodeData({ approvalReason: event.target.value })}
                          />
                        </Form.Item>
                        <Form.Item label="有效期（分钟）">
                          <InputNumber
                            min={1}
                            max={1440}
                            value={Number(selectedNode.data?.expiresMinutes ?? 60)}
                            onChange={(value) => updateSelectedNodeData({ expiresMinutes: value ?? 60 })}
                          />
                        </Form.Item>
                      </Form>
                    )}
