...
export interface NodeDefinition {
  ...
  outputSchema: string;
  configSchema: string;
  riskLevel?: 'READ_ONLY' | 'LOW_RISK' | 'HIGH_RISK' | 'GOVERNANCE';
}
 ...
 * 执行工作流
 */
export const executeWorkflow = (id: number, inputData: string): Promise<ApiResult<ExecutionResponse>> => {
...
export const executeWorkflowStream = async (
  ...
  const url = buildBackendUrl(
    `/api/workflows/${id}/execute/stream?inputData=${encodeURIComponent(inputData)}`
      + `&ticket=${encodeURIComponent(ticketResult.data.ticket)}`
  );
