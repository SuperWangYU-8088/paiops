...
public class WorkflowEngine implements WorkflowExecutor {
    ...
    @Autowired
    private NodeSafetyPolicy nodeSafetyPolicy;

    @Value("${paiops.execution.default-node-timeout-seconds:300}")
    private int defaultNodeTimeoutSeconds;

    private final ExecutorService nodeExecutionPool = Executors.newVirtualThreadPerTaskExecutor();
    ...
    public ExecutionResponse executeExisting(Workflow workflow,
                                             String inputData,
        ...
        return executeNodes(record, workflow.getId(), inputData, sortedNodes, indexes,
                new ResumeState(), startTime, eventCallback);
    }
    ...
    private ExecutionResponse executeNodes(ExecutionRecord record,
        ...
        try {
            for (WorkflowNode node : sortedNodes) {
                assertNotCancelled(record.getId());
                if (!startReached) {
                ...
                // 运行时字段只在内存中传给治理节点，不会写入节点输出或审计详情。
                nodeInput = new LinkedHashMap<>(nodeInput);
                nodeInput.put("__executionId__", record.getId());
                nodeInput.put("__flowId__", flowId);
                nodeSafetyPolicy.assertExecutionAllowed(node, nodeInput);
                ...
                try {
                    ...
                    Map<String, Object> rawOutput = executeNodeWithTimeout(
                            record.getId(), executor, node, nodeInput, eventCallback
                    );
                    ...
                    log.info("节点等待审批: {}", node.getId());
                    nodeResult.setStatus("WAITING_APPROVAL");
                    status = "WAITING_APPROVAL";
                    markSnapshotWaitingApproval(snapshot);
                    if (eventCallback != null) {
                        eventCallback.accept(ExecutionEvent.nodeProgress(
                                node.getId(), node.getType(), e.getMessage(), Map.of("status", "WAITING_APPROVAL")
                        ));
                    }
                    throw e;
                } catch (ExecutionCancelledException e) {
                    nodeResult.setStatus("CANCELED");
                    status = "CANCELED";
                    errorMessage = e.getMessage();
                    markSnapshotCanceled(snapshot);
                    throw e;
                } catch (Exception e) {
    ...
    private Map<String, Object> executeNodeWithTimeout(Long executionId,
                                                       NodeExecutor executor,
                                                       WorkflowNode node,
                                                       Map<String, Object> nodeInput,
                                                       Consumer<ExecutionEvent> eventCallback) throws Exception {
        int timeoutSeconds = resolveNodeTimeoutSeconds(node);
        // Java 21 虚拟线程允许每个节点独立设置超时，而不会为大量等待型 HTTP
        // 节点长期占用平台线程。超时后先中断，再由节点自身关闭网络资源。
        Future<Map<String, Object>> future = nodeExecutionPool.submit(
                () -> executor.execute(node, nodeInput, eventCallback)
        );
        ...
        try {
            while (true) {
                ...
                if (remainingNanos <= 0) {
                    future.cancel(true);
                    throw new IllegalStateException("节点执行超时（" + timeoutSeconds + " 秒）");
                }
                try {
                    // 最多等待一秒便重新检查一次数据库取消标记，使长时间 HTTP/轮询节点也能及时退出。
                    long pollNanos = Math.min(remainingNanos, TimeUnit.SECONDS.toNanos(1));
                    return future.get(pollNanos, TimeUnit.NANOSECONDS);
            ...
            throw new IllegalStateException("节点执行异常", cause);
    ...
    private int resolveNodeTimeoutSeconds(WorkflowNode node) {
        Object value = node.getData() == null ? null : node.getData().get("nodeTimeoutSeconds");
        int timeout = value instanceof Number number
                ? number.intValue()
                : defaultNodeTimeoutSeconds;
        return Math.max(1, Math.min(timeout, 3600));
    }

    private void assertNotCancelled(Long executionId) {
        ExecutionRecord latest = executionRecordMapper.selectById(executionId);
        if (latest != null && Integer.valueOf(1).equals(latest.getCancelRequested())) {
    ...
    private void markSnapshotWaitingApproval(ExecutionSnapshot snapshot) {
        if (executionSnapshotMapper == null || snapshot == null || snapshot.getId() == null) {
            return;
        }
        snapshot.setStatus("WAITING_APPROVAL");
        snapshot.setErrorMessage(null);
        executionSnapshotMapper.updateById(snapshot);
    }

    private void markSnapshotCanceled(ExecutionSnapshot snapshot) {
        if (executionSnapshotMapper == null || snapshot == null || snapshot.getId() == null) {
            return;
        }
        snapshot.setStatus("CANCELED");
        snapshot.setErrorMessage("执行任务已取消");
        snapshot.setCompletedAt(LocalDateTime.now());
        executionSnapshotMapper.updateById(snapshot);
    }

    @PreDestroy
    public void closeNodeExecutionPool() {
        ...
        if (value instanceof Map<?, ?> map) {
            ...
            for (Map.Entry<?, ?> entry : map.entrySet()) {
                ...
                if ("__nodeOutputs__".equals(key)
                        || "__executionId__".equals(key)
                        || "__flowId__".equals(key)
                        || "__actor__".equals(key)) {
                    continue;
                }
