...
class ConditionBranchRoutingTest {
    ...
    void setUp() {
        ...
        // 该测试只验证 DAG 条件分支，不重复验证安全策略。模拟节点类型不属于生产节点，
        // 因此注入无副作用的策略替身；真实安全规则由 NodeSafetyPolicyTest 单独覆盖。
        ReflectionTestUtils.setField(engine, "nodeSafetyPolicy", mock(NodeSafetyPolicy.class));
    }
    ...
    void testConditionBranchSelectsFirstBranch() {
        ...
        assertEquals("SUCCESS", response.getStatus(), response.getErrorMessage());
